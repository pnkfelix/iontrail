...
TypedDatum::attach(JSObject &datum, uint32_t offset)
{
    ...
    JS_ASSERT(datum.getReservedSlot(JS_TYPEDOBJ_SLOT_OWNER).isObject());

    // find the location in memory
    uint8_t *mem = TypedMem(datum) + offset;

    // find the owner, which is often but not always `datum`
    JSObject &owner = datum.getReservedSlot(JS_TYPEDOBJ_SLOT_OWNER).toObject();

    setPrivate(mem);
    setReservedSlot(JS_TYPEDOBJ_SLOT_OWNER, ObjectValue(owner));
}
...
{
    JS_ASSERT(IsTypedDatum(*datum));
    JS_ASSERT(offset <= BlockSize(*datum));
    JS_ASSERT(offset + TypeSize(*type) <= BlockSize(*datum));
